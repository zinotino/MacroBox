...
; ===== DIAGNOSTIC LOGGING =====
Stats_LogDiagnostic(message) {
    global workDir
    diagnosticLog := workDir . "\stats_diagnostic.log"
    timestamp := FormatTime(A_Now, "yyyy-MM-dd HH:mm:ss")
    logEntry := timestamp . " - " . message . "`n"

    try {
        FileAppend(logEntry, diagnosticLog, "UTF-8")
    } catch {
        ; Silent fail for diagnostic logging
    }
}
...
ReadStatsFromCSV(filterBySession := false) {
    ...
    ; DIAGNOSTIC: Log stats reading operation
    Stats_LogDiagnostic("ReadStatsFromCSV called - sessionId: " . sessionId . ", totalActiveTime: " . totalActiveTime . ", filterBySession: " . filterBySession)
    ...
    try {
            ...
            if (!filterBySession || fields[2] = sessionId) {
                ...
                try {
                    ...
                    if (fields.Length >= 23) {
                        ...
                        ; DIAGNOSTIC: Log degradation parsing
                        if (A_Index = 2) { ; Log only first few rows to avoid spam
                            Stats_LogDiagnostic("Parsed CSV row - type:" . execution_type . " degradations: smudge:" . smudge . " glare:" . glare . " splashes:" . splashes . " partial:" . partial . " full:" . full . " flare:" . flare . " rain:" . rain . " haze:" . haze . " snow:" . snow . " clear:" . clear)
                        }
        ...
        ; Include current session's active time in total
        currentSessionActive := totalActiveTime
        if (lastActiveTime > 0 && !breakMode) {
            currentSessionActive += A_TickCount - lastActiveTime
        }
        totalSessionActive += currentSessionActive

        ; DIAGNOSTIC: Log session active time calculation
        Stats_LogDiagnostic("Session active time calc - historical: " . (totalSessionActive - currentSessionActive) . "ms, current: " . currentSessionActive . "ms, total: " . totalSessionActive . "ms")
        ...
        ; DIAGNOSTIC: Log final aggregated stats
        Stats_LogDiagnostic("Final stats - executions:" . stats["total_executions"] . " boxes:" . stats["total_boxes"] . " active_time:" . stats["session_active_time"] . "ms avg_time:" . stats["average_execution_time"] . "ms boxes/hr:" . stats["boxes_per_hour"] . " exec/hr:" . stats["executions_per_hour"])
        Stats_LogDiagnostic("Degradation totals - smudge:" . stats["smudge_total"] . " glare:" . stats["glare_total"] . " splashes:" . stats["splashes_total"] . " partial:" . stats["partial_blockage_total"] . " full:" . stats["full_blockage_total"] . " clear:" . stats["clear_total"])
...
ProcessDegradationCounts(executionData, degradationString) {
    ...
    ; For JSON profiles: set count to 1 for the selected degradation type (not increment)
    ...
    for degradationType in degradationTypes {
        ...
        ; For JSON profiles, each execution counts as 1 for the selected type
...
RecordExecutionStats(macroKey, executionStartTime, executionType, events, analysisRecord := "") {
    ...
    ; DIAGNOSTIC: Log execution data being recorded
    degradationSummary := "smudge:" . executionData["smudge_count"] . " glare:" . executionData["glare_count"] . " splashes:" . executionData["splashes_count"] . " partial:" . executionData["partial_blockage_count"] . " full:" . executionData["full_blockage_count"] . " flare:" . executionData["light_flare_count"] . " rain:" . executionData["rain_count"] . " haze:" . executionData["haze_count"] . " snow:" . executionData["snow_count"] . " clear:" . executionData["clear_count"]
    Stats_LogDiagnostic("Recording execution - type:" . executionType . " boxes:" . executionData["total_boxes"] . " time:" . execution_time_ms . "ms active:" . current_session_active_time_ms . "ms degradations: " . degradationSummary)
