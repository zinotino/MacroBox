...
; ===== SET ACTIVE CANVAS =====
; Sets the active canvas mode and updates legacy globals for compatibility
Canvas_SetActiveCanvas(mode) {
    global CanvasState
    global userCanvasLeft, userCanvasTop, userCanvasRight, userCanvasBottom, isCanvasCalibrated

    if (mode = "wide" && CanvasState.wide.calibrated) {
        userCanvasLeft := CanvasState.wide.left
        userCanvasTop := CanvasState.wide.top
        userCanvasRight := CanvasState.wide.right
        userCanvasBottom := CanvasState.wide.bottom
        isCanvasCalibrated := true
    } else if (mode = "narrow" && CanvasState.narrow.calibrated) {
        userCanvasLeft := CanvasState.narrow.left
        userCanvasTop := CanvasState.narrow.top
        userCanvasRight := CanvasState.narrow.right
        userCanvasBottom := CanvasState.narrow.bottom
        isCanvasCalibrated := true
    } else if (mode = "user" && CanvasState.user.calibrated) {
        userCanvasLeft := CanvasState.user.left
        userCanvasTop := CanvasState.user.top
        userCanvasRight := CanvasState.user.right
        userCanvasBottom := CanvasState.user.bottom
        ...
        ; Fallback to defaults if not calibrated
        userCanvasLeft := 0
        userCanvasTop := 0
