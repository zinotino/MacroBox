...
MacroMaster is a comprehensive AutoHotkey v2.0 macro recording and playback system designed for offline data labeling workflows. The system uses a modular architecture with multiple visualization layers and SQLite-based analytics.

### Key Characteristics

- **Modular Design**: 20+ separate AHK modules with clear separation of concerns
- **Multi-Layer Visualization**: HBITMAP, PNG, and Plotly dashboard systems
- **Dual Canvas Support**: Wide (16:9) and narrow (4:3) aspect ratio handling
- **Real-time Statistics**: SQLite-backed analytics with timeline filtering
...
graph TB
    subgraph "User Interface Layer"
        ...
        STATS[Stats Dashboard<br/>Plotly Charts<br/>Timeline Filters]
    ...
    subgraph "Data & Analytics"
        ...
        SQLITE[(SQLite Database<br/>39 executions<br/>43 degradations<br/>5 sessions<br/>0.09 MB)]
...
sequenceDiagram
    ...
    participant C as Core.ahk
    participant S as Stats.ahk
    participant P as generate_dashboard.py
    participant B as Browser

    U->>C: Click Stats button
    C->>S: Launch dashboard generation
    S->>P: Execute Python script
    P->>P: Query SQLite database
    P->>P: Generate Plotly charts
    P->>P: Create HTML dashboard
    P->>C: Return dashboard path
    C->>B: Open in default browser
    B->>U: Display interactive charts
```
...
graph TD
    ...
    subgraph "Statistics Storage"
        C[master_stats.csv<br/>Execution records<br/>CSV format]
        D[(macromaster_stats.db<br/>SQLite database<br/>Indexed tables)]
    end
...
```mermaid
erDiagram
    executions ||--o{ degradations : contains
    executions {
        id INTEGER PK
        timestamp DATETIME
        session_id TEXT
    ...
    executions ||--o{ sessions : belongs_to
```
...
| | Stats recording | <10ms | CSV + SQLite |
| **Database** | Query execution | <100ms | With indexes |
| | Dashboard generation | 2-10s | Full dataset |
...
- **Execution History**: 100K+ records in SQLite
...
- **No External Executables**: Pure AHK + Python for dashboard
...
- **Phase 3**: Direct database writes, real-time updates
...
| **Statistics** | `Stats.ahk` | Data collection, CSV output |
| **Dashboard** | `stats/generate_dashboard.py` | Analytics visualization |
