...
- **Statistics Integration**: Real-time stats collection with CSV storage
...
├── data\              # Configuration and CSV stats files
├── thumbnails\        # Button visualization cache (PNG)
...
#### File Paths
```autohotkey
global sessionStatsCSV := workDir . "\session_stats.csv"
...
- `AppendToCSV()` - Writes execution data to CSV files
- `ReadStatsFromCSV()` - Loads historical data for display
...
#### Data File Variables
```autohotkey
global sessionStatsFile := workDir . "\session_stats.csv"
global masterStatsFile := workDir . "\master_stats.csv"
