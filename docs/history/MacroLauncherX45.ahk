...
CreateMacroVisualization(macroEvents, buttonDims) {
    ...
    ; Extract recordedMode from macroEvents
    recordedMode := macroEvents.HasOwnProp("recordedMode") ? macroEvents.recordedMode : "Wide"
    ...
    try {
        ...
        ; Draw macro boxes optimized for button dimensions with recordedMode
...
; Uses recordedMode to determine canvas configuration for letterboxing
; - Wide recorded macros → Use WIDE canvas config → STRETCH to fill thumbnail (no black bars)
; - Narrow recorded macros → Use NARROW canvas config → Black bars based on configured narrow aspect ratio
; - Canvas choice based on RECORDED MODE, not content analysis
; - Clean visualization without indicators for maximum aesthetic appeal
DrawMacroBoxesOnButton(graphics, buttonWidth, buttonHeight, boxes, recordedMode := "Wide") {
    ...
    ; PRIORITIZE RECORDED MODE: Use recordedMode to determine canvas configuration
    ; This ensures letterboxing appears for narrow recorded macros regardless of content

    if (recordedMode = "Narrow") {
        ; NARROW RECORDED MACRO: Always use narrow canvas configuration for letterboxing
        useWideCanvas := false
        useNarrowCanvas := true
        useLegacyCanvas := false
    } else {
        ; WIDE RECORDED MACRO: Use wide canvas configuration (default behavior)
        useWideCanvas := true
        useNarrowCanvas := false
        useLegacyCanvas := false
    }
    
    ; ENHANCED DIAGNOSTIC: Log canvas selection based on recordedMode
    debugInfo := "Canvas Selection: "
    debugInfo .= "RecordedMode:" . recordedMode . " "
    debugInfo .= "Recorded(" . Round(minX) . "," . Round(minY) . " to " . Round(maxX) . "," . Round(maxY) . ") "
    debugInfo .= "Aspect:" . Round(recordedAspectRatio, 2) . " "

    if (useWideCanvas) {
        debugInfo .= "→ WIDE canvas (no letterboxing)"
    } else if (useNarrowCanvas) {
        debugInfo .= "→ NARROW canvas (letterboxing enabled)"
    } else {
        debugInfo .= "→ LEGACY fallback"
    }
...
ForceStartRecording() {
    global recording, currentMacro, macroEvents, currentLayer, mainGui, pendingBoxForTagging, annotationMode
    ...
    macroEvents[currentMacro] := {recordedMode: annotationMode, events: []}
    UpdateStatus("🎥 DEBUG: Recording started in " . annotationMode . " mode")
    pendingBoxForTagging := ""
...
MouseProc(nCode, wParam, lParam) {
    ...
    if (!macroEvents.Has(currentMacro))
        macroEvents[currentMacro] := {recordedMode: "Wide", events: []}

    local events := macroEvents[currentMacro].events
...
AssignToButton(buttonName) {
    ...
    ; Copy the entire macroEvents object (including recordedMode and events)
    macroEvents[layerMacroName] := macroEvents[currentMacro]
        ...
        for line in lines {
            ...
            if (InStr(line, "=")) {
                ...
                if (currentSection = "General") {
                    ...
                    ; Parse macro data with backwards compatibility
                    if (value != "") {
                        ; Split by | separator
                        eventLines := StrSplit(value, "|")
                        
                        ; Check if first element is a mode (new format) or event (old format)
                        if (eventLines.Length > 0 && (eventLines[1] = "Wide" || eventLines[1] = "Narrow")) {
                            ; New format: recordedMode|event1|event2|...
                            recordedMode := eventLines[1]
                            eventStartIndex := 2
                        } else {
                            ; Old format: event1|event2|... (default to Wide)
                            recordedMode := "Wide"
                            eventStartIndex := 1
                        }
                        
                        ; Initialize macroEvents as object with recordedMode and events array
                        macroEvents[key] := {recordedMode: recordedMode, events: []}
                        loadedEvents := 0
                        
                        ; Process events starting from eventStartIndex
                        Loop (eventLines.Length - eventStartIndex + 1) {
                            eventIndex := eventStartIndex + A_Index - 1
                            eventLine := eventLines[eventIndex]
                            ...
                            if (parts[1] = "jsonAnnotation") {
                                ...
                                macroEvents[key].events.Push(event)
                                loadedEvents++
                            } else {
                                event := {type: parts[1]}
